...
export function processWeatherData(data: WeatherResponse, runHour: number, runMinute: number): RunDay[] {
  const now = new Date();
  ...
  // Round to nearest hour for API hourly data lookup
  const apiHour = runMinute >= 30 ? (runHour + 1) % 24 : runHour;

  // Future-only logic: include today only if the run is more than 60 minutes away
  const startIndex = data.days.findIndex((d) => {
    if (d.datetime === todayStr) {
      const minutesUntilRun = runTotalMinutes - currentTotalMinutes;
      return minutesUntilRun > 60;
    }
  ...
  const runDays: RunDay[] = days.map((day, i) => {
    const hourData = getHourData(day, apiHour);
